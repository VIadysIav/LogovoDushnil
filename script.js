...
// Переинициализируем просмотрщик изображений при каждом переходе на страницу
function reinitializeImageViewers() {
    // Настраиваем просмотрщик для главной страницы
    setupImageViewersForContainer(document.querySelector('.home-page'));
        
    // Настраиваем просмотрщик для страницы с лентой
    setupImageViewersForContainer(document.querySelector('.feed-page'));
        
    // Настраиваем просмотрщик для каждой категории
    document.querySelectorAll('.category-page').forEach(categoryPage => {
        setupImageViewersForContainer(categoryPage);
    });
        
    // Настраиваем просмотрщик для полноэкранной страницы
    setupImageViewersForContainer(document.getElementById('fullscreen-page-container'));
}
    
// Вызываем инициализацию при загрузке страницы
reinitializeImageViewers();
    
// Добавляем обработчик для переинициализации при изменении URL
window.addEventListener('popstate', function() {
    setTimeout(reinitializeImageViewers, 500);
});
    
// Переопределяем функцию showPage, чтобы она вызывала reinitializeImageViewers
...
// Добавим вызов setupImageViewers после загрузки категории
function loadCategory(categoryName, itemId = null) {
    ...
    // Добавляем вызов setupImageViewers для страницы категории
    setupImageViewersForContainer(categoryPage);
...
// Функция для настройки просмотрщика изображений, которая будет вызываться для всех страниц
function setupImageViewers() {
    // Найдем все изображения с классом post-image на всех страницах сайта
    const images = document.querySelectorAll('.post-image');
    const modal = document.querySelector('.image-modal');
    
    if (!modal) {
        console.error('Модальное окно для просмотра изображений не найдено');
        return;
    }
    ...
    if (!modalContent || !closeButton || !modalImg) {
...
// Функция для открытия полноэкранной страницы
function openFullscreenPage(item, updateHistory = true) {
    ...
    setTimeout(() => {
        ...
        // Применяем обработчики для изображений внутри полноэкранной страницы
        // Это ключевое исправление для вашей проблемы
        const fullscreenImages = pageContainer.querySelectorAll('.post-image');
...
// Модифицированная функция инициализации полноэкранной страницы
function initFullscreenPageSystem() {
    ...
    if (!container) {
        ...
        backButton.innerHTML = '<b><h3>🠔 Назад</h3></b>'; // Используем HTML для жирного текста
...
// НОВАЯ ФУНКЦИЯ: Настройка просмотрщика изображений для конкретного контейнера
function setupImageViewersForContainer(container) {
    ...
    images.forEach(img => {
        ...
        newImg.onclick = function(e) {
            ...
            // Запускаем рендеринг браузера для корректных CSS анимаций
            setTimeout(function() {
                modal.classList.add('show');
